    ...
    {
        {
			...
			if (m_CurrentSession)
			{
				...
				{
					STUDY_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Log::GetCoreLogger()) 
				{
					STUDY_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
    }
